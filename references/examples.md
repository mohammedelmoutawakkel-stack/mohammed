# Humanizer: Full Pattern Examples

Worked before/after examples for all 33 patterns in `SKILL.md`. The skill body keeps every
pattern's **Words to watch** and **Problem** line inline (enough to detect and fix each one) plus
compact inline examples for the highest-value patterns; this file holds the complete example set,
including the ones already inline, for reference when a transformation is ambiguous.

Headings match the numbering and names in `SKILL.md` exactly.
