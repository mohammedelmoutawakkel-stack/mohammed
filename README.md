...
- **3.0.1** - Added a Cursor plugin manifest so this repository can load as a Cursor plugin and be submitted to the Cursor Marketplace. Cursor discovers the root `SKILL.md`; the manifest omits a `skills` path so that discovery keeps working. No change to the 25 patterns.
