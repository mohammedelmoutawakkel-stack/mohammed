...
if pattern_numbers != list(range(1, 41)):
    raise SystemExit(f"Number SKILL.md patterns from 1 through 40: {pattern_numbers}")

readme_numbers = {
    int(number) for number in re.findall(r"(?m)^\| ([0-9]+) \|", README)
}
if readme_numbers != set(range(1, 41)):
    raise SystemExit("List patterns 1 through 40 in the README table")
